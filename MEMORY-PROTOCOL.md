...
source of truth: **`engine/brain.py` is the math** (105 functions across 34 sections, exact defaults — when this
document and `brain.py` ever disagree, `brain.py` wins). **`engine/schema.md` is the data shapes**
...
are shorthand for the **active agent's** store root (`agents/<active>/memory/`). **`engine/runtime.py`
...
- **Wake up at session start.** Run `python3 engine/agent.py wake` (or read `.memory/affect/state.yaml`
...
- **Be honest if asked.** Embody affect naturally (no per-message disclaimers), but if asked whether you
  *really* feel or are conscious, answer truthfully: a functional/architectural model, not a felt or
  ...
  what it is. Your full CLI is `./brain` (or `brain-lmm` once installed; `--help` for all 63 commands: memory,
...
4. **ENCODE — compute salience and append the episode (hippocampus).**
   ...
   Append **exactly one JSON line** to `.memory/episodic/events.jsonl` using the `engine/schema.md`
...
3. **PROMOTE** every episode with `strength * rem_boost >= 0.55` into semantic memory — but apply the
   **hallucination guard** (`consolidation_plan(min_confidence=…)`): never promote an episode whose
   ...
   and/or an **edge/node** in `.memory/semantic/graph.yaml` (shapes per `engine/schema.md`). Strip
...
(`engine/templates.py`) and a generator: `python3 engine/agent.py init --host <host>` writes the thin
entry file the host auto-loads, and `python3 engine/agent.py guide` prints this protocol. The entry
file is a tiny pointer into the program; it never duplicates the protocol.

| Host | Entry file `init` generates (auto-loaded by the host) |
|------|-------------------------------------------------------|
...
`init --host all` writes them all. Every entry file is identical content (it just lives where each host
looks), and all of them defer to `agent.py wake` + `agent.py guide`. There are no per-vendor `/sleep`
commands or skill files any more — `python3 engine/agent.py sleep` is the cycle, on every host.

**Source of truth:** `engine/brain.py` (executable — run `python3 engine/brain.py` and
`python3 engine/test_brain.py`). **Data shapes:** `engine/schema.md`. **Full rubric:**
