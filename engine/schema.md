...
> `.memory/…` as shorthand for that per-agent store root — substitute `agents/<active-agent>/memory/`.
...
fades). `source` is the episode id (or `"reflection"` for synthesized clusters). `engine/runtime.py`
...
All `engine/brain.py` functions operate on these shapes. Times are unix seconds except
