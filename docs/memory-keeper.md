...
> are shorthand for the **active agent's** store root. The CLI (`engine/agent.py` / `./brain`) resolves
> it for you — `./brain wake|remember|recall|sleep|…` act on the active agent; `--agent <name>` overrides.
Every number, threshold, and formula below is taken from the executable source of truth,
`engine/brain.py`. The canonical data shapes (Episode, Affect state, Semantic graph) live in
`engine/schema.md`. The condensed per-task loop and the `/sleep` cycle live in `MEMORY-PROTOCOL.md`.
...
## 4. The functions (each in `engine/brain.py`)
...
21. **`brier_score` / `metacog_sensitivity` / `label_stability` / `recall_accuracy` /
    ...
    (affect/cognitive groundable, felt-body band never). Run via `engine/test_brain.py`; see `docs/eval.md`.
...
22. **`consciousness_indicators(modules)`** + `INDICATOR_THEORY` / `SHIPPED_MODULES` / `CONSCIOUSNESS_CAVEAT`
    ...
    *Necessary-NOT-sufficient, theory-relative, assumes contested computational functionalism (Butlin et
    ...
    Orch-OR denies the premise. A transparency map of architecture, NOT a test of experience — brain-lmm
...
See `engine/schema.md` for the exact JSON/YAML shapes. (Note: the `NetworkLayer` content in
...
These match the `python3 engine/brain.py` demo exactly. Reproduce them to sanity-check any port or
change.
...
`python3 engine/test_brain.py`). **Data shapes:** `engine/schema.md`. **Per-task loop & /sleep
