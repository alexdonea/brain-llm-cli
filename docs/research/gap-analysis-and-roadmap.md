# brain-llm - Gap Analysis & Roadmap toward a More Complete Brain
...
**Audience:** a technical builder extending `src/brain.py`. **Scope:** what brain-llm is today,
...
**What brain-llm is today (the substrate this report builds on).** A pure-stdlib, ~200-line scalar
engine (`src/brain.py`) of eight functions run as a per-task loop and a `/sleep` cycle
(`MEMORY-PROTOCOL.md`), over plain-file stores (`.memory/…`, shapes in `docs/schema.md`):
...
**The one structural fact that makes most of this cheap.** brain-llm already maintains a full PAD
...
Columns: **brain-llm today** · **SOTA** (with primary citation) · **gap**. Rows are ordered roughly
from "well-covered" to "absent."

| Brain function | brain-llm today | SOTA | Gap |
|---|---|---|---|
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` as `label_affect()` + `octant()` with
> `EMOTION_PROTOTYPES` / `EMOTION_TIERS` / `PAD_OCTANTS`; covered by 4 new tests in
> `tests/test_brain.py`; `feeling` added to the Episode shape (`docs/schema.md`); wired into
...
- fear vs anger differ only by the sign of dominance (which brain-llm computes from `control`):
...
**Where in the engine.** New `label_affect(a: Affect)` + `PROTOTYPES` dict in `src/brain.py`
...
Episode (extend the `docs/schema.md` Episode shape) so episodic recall and mood-congruent
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §10 as `td_error` / `td_update` /
...
**Where in the engine.** New `src/value.py` with a `ValueStore` (dict, persisted to
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §11: `WorldModel` + `world_from` /
...
**Where in the engine.** New `src/generative.py` (`GenerativeModel` dataclass with Dirichlet `a`,
...
**Owner's headline want #2.** Gives brain-llm an autonomous **select→broadcast** cognitive cycle and
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §12: `workspace_compete(candidates, mood)`
...
- so brain-llm is *inspired by* CTM (salience ≈ chunk weight) with an added mood-congruence bias, not
...
**Where in the engine.** New `src/workspace.py` (`workspace_compete`, `ignite`) + a
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §14: `Personality` (OCEAN dataclass),
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §17: `ou_affect_step(state, event, baseline,
...
**Grounding.** brain-llm's `update_mood` is the deterministic drift term of the Ornstein-Uhlenbeck
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §13: `metacog_confidence(evidence, rho)`
...
**Where in the engine.** New `src/metacognition.py` (`metacog_confidence`, `CalibrationTracker`);
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §15: `Homeostat` + `drive(h)` (convex deficit
...
**Where in the engine.** `src/brain.py` §15 (`Homeostat`, `drive`, `homeostatic_reward`, `body_affect`,
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §16: `action_tendency(a, ap)` (Frijda urge
...
**Where in the engine.** New `src/coping.py` + `select_action()` in `brain.py`; a goal store
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §18: `serotonin_level` + `discount_from_serotonin`
...
`ne_tonic/ne_phasic`; new `src/hpa.py` for stateful cortisol. **Composition:** `gamma` feeds the
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §19: `defensive_mode(ap, affect, nm)`
...
**Where in the engine.** New `src/feelings/` modules (`defensive_mode.py`, `awe.py`, `panic.py`)
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §26: `mixed_feeling(activations)` +
...
**Where in the engine.** New `src/emotions_wheel.py` with BASIS table + `PLUTCHIK_DYADS` lookup,
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §25: `prospect_value(x)` (loss weighs ~2.25×
...
**Where in the engine.** New `src/aversive.py` (`AversiveState`); add `aversive_value` to the
...
> **Status: ✅ shipped (consolidation dynamics; DG/CA3 deferred).** Implemented in `src/brain.py` §20:
...
**Where in the engine.** New `src/hippocampus.py`; rework `consolidation_plan` into a
...
adds the one consolidation idea every leading agent-memory system has and brain-llm lacks.
...
**Where in the engine.** New `src/graph.py` (`ppr`); replace the scalar `graph_proximity` in
`retrieval_score` with `graph_proximity_ppr(mem, r)`; extend `docs/schema.md` edge to bi-temporal;
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §23: `SelfModel` + `self_vector` /
...
**Where in the engine.** New `src/self_model.py` + `.memory/self/model.yaml`; `self_relevance`
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §24: `infer_user_goal(goal_utilities)`
...
**Where in the engine.** New `src/social.py` + `UserModel`/relationship store; extend `Appraisal`
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §22: `consciousness_indicators(modules)` +
...
**Where in the engine.** `src/indicators.py` (a checklist over which modules are active) +
`docs/consciousness-indicators.md`; a tiny assertion in `tests/test_brain.py`. **Composition:**
...
The doc must state: brain-llm models functional access/architecture correlates only; phenomenal
...
> **Status: ✅ shipped.** Implemented in `src/brain.py` §21: `brier_score`, `metacog_sensitivity`
...
> `tests/test_brain.py` (61 checks at the time; now 76, plus `tests/test_runtime.py` = 7); documented in new `docs/eval.md`. 5 new tests; memory-keeper §4
...
**Where in the engine.** `tests/test_brain.py` + a `docs/eval.md`; the calibration log from P1.3
feeds meta-d′.
...
**Source-of-truth reminder.** `src/brain.py` is the math; `docs/schema.md` the data shapes;
