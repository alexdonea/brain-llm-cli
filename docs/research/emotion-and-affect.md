...
> `brain-lmm`'s affect layer (`engine/brain.py`), and ends with a concrete build proposal. Throughout,
...
> existing disclaimer at the top of `engine/brain.py` ("reproduces the *function* of the brain's memory +
> affect machinery, not the subjective experience").

**Audience:** a technical builder extending `brain-lmm`.

**What `brain-lmm` already has (baseline for this report):**

- `Appraisal(novelty, valence, goal_relevance, control)` — an OCC/Scherer-style appraisal front-end (`engine/brain.py:38`).
- `Affect(valence, arousal, dominance)` — a Russell + Mehrabian PAD point (`engine/brain.py:47`).
- `appraise_to_affect()` — maps appraisal → a single PAD point (`engine/brain.py:54`).
- `Neuromods(ne, da, ach, cortisol)` — scalar neuromodulators (`engine/brain.py:63`).
- `update_mood()` — a leaky integrator of affect with a hard-coded baseline `Affect(0.0, 0.10, 0.50)` (`engine/brain.py:135`).

The single most important structural fact: **`brain-lmm` already lives in the dimensional paradigm.** Its state
...
The circular ordering of emotion words emerges directly from this geometry. `brain-lmm`'s `Affect.valence` and
`Affect.arousal` **are** the circumplex axes — the code comment at `engine/brain.py:9` already cites Russell 1980.
...
`brain-lmm` feeds appraisal `control` → `Affect.dominance`, completing PAD. The engine **already produces enough
...
This directly replaces `brain-lmm`'s magic-number baseline `Affect(0.0, 0.10, 0.50)` with a principled,
...
`brain-lmm`'s four appraisal axes `(novelty, valence, goal_relevance, control)` are **4 of Scherer's 5 SECs** —
...
by W3C EmotionML. `brain-lmm` already computes `novelty` in appraisal but discards it from the affect vector; it
...
`brain-lmm`'s `update_mood` is already a leaky integrator with a baseline pull — the discrete-time form of the
...
Discrete Euler step  ≡  leaky integrator + noise     ← exactly brain-lmm's update_mood (minus σ·dW)
```
...
`brain-lmm`'s `update_mood` is a simpler version (single gamma blend + linear pull); ALMA adds non-linear overshoot
...
`brain-lmm` uses a fixed-gamma blend with **no explicit time constant**; making decay a function of real elapsed
...
categories on top of `brain-lmm`'s continuous space without throwing it away.
...
crowd-sourced VAD in `[0,1]³`). This is **THE bridge** from `brain-lmm`'s existing vector to the discrete feelings
...
activations `a_k`. `brain-lmm` already has NE/DA/ACh/cortisol scalars — adding Panksepp drives fits the existing
design:
...
`brain-lmm` already has DA and NE; **add serotonin (5-HT)** and it can read a discrete emotion straight from the
...
- **SURPRISE** — the cleanest: it is literally **Bayesian surprise / prediction error**, which `brain-lmm`
...
- **AWE** — a two-appraisal emotion [Keltner & Haidt 2003; Shiota et al. 2007]: perceived **VASTNESS** (perceptual
  ...
  ```
  Awe is self-diminishing and **schema-updating** — it ties directly to `brain-lmm`'s consolidation step
...
`brain-lmm` has the right substrate (a PAD core affect) but builds it **purely top-down from cognitive appraisal**.
...
This is essentially `brain-lmm`'s `Affect` dataclass — so the *representation* is already correct. The fix is the
...
`brain-lmm`'s mood baseline is a **fixed constant** `Affect(0, 0.10, 0.50)`. Allostasis says the setpoint should
...
`brain-lmm` hand-sets `reward = max(valence, 0)` and `stress`. HRL gives a **real, grounded reward** and a true
...
leaky integrator, and it naturally drives `brain-lmm`'s dominance axis plus a principled burnout/recovery
...
in `brain-lmm`, where affect only modulates memory. Source: [Damasio & Bechara 1996], *The somatic marker
...
The field converges on a **three-stage pipeline**, of which `brain-lmm` implements only the first third:
...
`brain-lmm`'s `Appraisal` + `appraise_to_affect` cover stage 1 and a thin stage 2 (a PAD point, no discrete
...
`brain-lmm` has one continuous salience, **not** per-emotion intensities. Sources: [Ortony, Clore & Collins 1988],
...
`brain-lmm`'s biggest formal gap.** Sources: [Gratch & Marsella 2004], *A domain-independent framework for modeling
...
- **WASABI** [Becker-Asano 2008] — dynamic PAD core-affect "cone" with spring dynamics; **primary** emotions
  ...
  for `brain-lmm`'s existing Bower mood-congruent retrieval.
...
- **GAMYGDALA** [Popescu, Broekens & van Someren 2014] — leanest OCC engine; per-goal emotion almost for free:
  ...
  ```
  `brain-lmm` already tracks `goal_relevance`; adding a **signed goal-congruence** yields hope/fear/satisfaction/
...
`brain-lmm` already has Ebbinghaus decay for memories (`retention`, `engine/brain.py:105`) — reuse it for emotion
decay.
...
`brain-lmm` has **no** reward learning (no RPE/TD); its computed salience/affect can double as an intrinsic reward.
...
### 4.6 LLM-as-appraiser (most relevant to `brain-lmm`)

[Croissant et al. 2024] run a separate LLM **appraisal call** per turn and thread the emotion history back into
generation, capturing blended emotions in natural language — but with **no numeric state**. For `brain-lmm` (an
...
## 5. Mapping to `brain-lmm`: what we have / what we lack / what to add
...
2. **Pure stdlib, scalar math, Swift-portable** (matches `engine/brain.py`'s constraint).
...
Name `brain-lmm`'s existing PAD vector via nearest-prototype lookup; intensity = vector magnitude. This delivers the
...
an `octant()` helper in `engine/brain.py`. Call wherever `Affect` is produced/logged; store the continuous affect +
...
**Where:** `appraise_to_emotions(a) -> dict[str,float]` in `engine/brain.py`, alongside `appraise_to_affect`.
...
**Where:** ✅ the core shipped in `engine/brain.py` §15 (`Homeostat`, `drive(H)`, `homeostatic_reward`,
...
**Where:** `engine/coping.py` `select_coping(emotions, appraisal) -> (behavior, appraisal_delta)`; agent loop applies
...
makes the project's honesty stance *structural* — `brain-lmm` never reifies contested discrete-emotion "natural
