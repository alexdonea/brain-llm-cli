...
> Every layer in `engine/brain.py` is a *claim*. This harness turns claims into *tests*. Without it,
> the additions are unfalsifiable. The functions live in `engine/brain.py` §21; the executable suite is
> `engine/test_brain.py` (run `python3 engine/test_brain.py` → `All checks passed`).
